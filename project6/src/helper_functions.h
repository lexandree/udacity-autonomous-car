...
/**
 * Struct representing one landmark observation measurement, moved to map.h
...
inline T1 dist(const T1 x1, const T1 y1, const T1 x2, const T1 y2) {
 ...
 * @param (x) LandmarkObs point of the probability density to compute
 * @param (m) LandmarkObs mean of the distribution
 * @param (s) variance array of the distribution
 * @output probability density at point x
 */
inline double normal_pdf(LandmarkObs x, LandmarkObs m, double s[]) {
	static const double two_pi = 2 * M_PI;
	double norm_factor = two_pi * s[0] * s[1];
	double probability = exp(-( pow(x.x - m.x, 2) / (2 * pow(s[0], 2)) + pow(x.y - m.y, 2) / (2 * pow(s[1], 2))));
	return probability/norm_factor;
}

/**
 * Computes the normal probability distribution.
 * @param (x) point of the probability density to compute
...
inline bool read_map_data(std::string filename, Map& map) {
  ...
  while (getline(in_file_map, line_map)) {
    ...
    LandmarkObs single_landmark_temp;
...
inline double normalize_angle(const double &phi) {
